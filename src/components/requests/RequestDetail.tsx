  ...
  return (
    <div className="container-page flex flex-col gap-5 pt-10">
      ...
      <section className="rounded-2xl bg-surface-page-surf1 p-5 sm:rounded-3xl sm:p-8">
        ...
        <div className="mt-4 flex flex-col gap-3 lg:flex-row">
          ...
          {/* Сумма получения считается У НАС из value и rate, а не берётся из
              поля amount апстрима. Для новых заявок это одно и то же число
              (маппер уже развернул семантику ядра в доменную), но на
              дев-контуре лежат записи, искажённые нашим прежним форматом
              отправки (№6729, 6735, 6763–6765 — тенге уходили в поле
              валюты, см. reserve.ts), — собственный пересчёт из value и
              rate показывает осмысленную сумму и для них. */}
          <AmountBox
      ...
      <div className="flex flex-col items-center gap-3 pb-6 sm:flex-row sm:justify-between">
        ...
        {/* Отмена недоступна у любой «восьмёрки» — и у действующей брони
            (согласованную сделку клиент отменить не может, решение
            заказчика), и у ещё не подтверждённой: ядро ставит статус 8 сразу
            при создании, и показывать на таком экране кнопку отмены заказчик
            запретил отдельно. Отменяемыми остаются заявки в статусе 0. */}
        {cancellable && !r.needsClientConfirmation && r.phase !== 'held' && r.status !== 8 && (
          <Button
            variant="surf2"
