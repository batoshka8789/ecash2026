...
export function AppBanner() {
  ...
  return (
    <div className="overflow-hidden bg-surface-page-surf3 pb-[env(safe-area-inset-bottom)] shadow-[0_4px_8px_0_rgb(0_0_0/4%),0_0_2px_0_rgb(0_0_0/8%)]">
      <div className="container-page py-6">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center">
          <div className="flex items-start gap-4 min-[480px]:gap-8 lg:flex-1 lg:items-center">
            <Logo className="shrink-0" />
            <p className="text-sm leading-[1.1] text-text-default lg:max-w-[309px] lg:font-medium">
              {t('text')}
            </p>
            <button
              type="button"
              onClick={dismiss}
              aria-label={t('close')}
              className="ml-auto inline-flex h-6 w-6 shrink-0 cursor-pointer items-center justify-center rounded-full text-text-default transition-colors hover:bg-comp-surface2-hover lg:hidden"
            >
...
function StoreChip({
  ...
  return (
    <span className="inline-flex h-[46px] flex-1 items-center justify-center gap-2 rounded-[20px] bg-btn-inverted pl-4 pr-6 text-sm font-medium leading-5 text-text-inverted opacity-80 lg:flex-none">
      {children}
