...
function RailCard({
  ...
  return (
    <motion.div
      whileHover={reduced ? undefined : { y: -6 }}
      /*
        Ширина — доля КАДРА, а не фиксированные пиксели из макета.
        Раньше карточка была жёстко 704 (пакет) или 580 (поддержка), а колонка
        на боевых ширинах равна 1280–1324. Целое число таких карточек в кадр
        не помещалось никогда, поэтому у правого края ВСЕГДА торчал огрызок
        следующей — именно это и читалось как «обрезано и некрасиво».

        Теперь в кадре ровно одна карточка до 768 и ровно две дальше: доля
        считается от ширины кадра за вычетом зазора между карточками (12 → 20
        → 40, те же значения, что у gap ряда). Правый край при этом совпадает
        с краем колонки — обрезать нечего, а свайп остался ровно тем же и
        листает по одному экрану за раз благодаря snap-mandatory.
      */
      className="w-full shrink-0 snap-start md:w-[calc((100%-20px)/2)] lg:w-[calc((100%-40px)/2)]"
    >
      <Spotlight
