  ...
  return (
    <motion.div
      ...
      /*
        ...
        «Поддержки», сняты напрямую с узла cards new в дампе Figma (не с
        рассчитанной доли контейнера). Внутренние padding 64 (p-16) и gap 44
        (gap-11) у Spotlight ниже — оттуда же, и их я не трогал.

        Предыдущий заход считал ширину долей кадра, чтобы в 1280 умещалось
        ровно два экземпляра без остатка. Это МЕНЯЛО пропорции карточки
        (620px вместо 704) — текст внутри не растянут stretch fill-x на 100%
        ширины, а Ecash в макете шириной 704 подобрал перенос строк под эту
        ширину. У чужой ширины перенос текста уже не совпадает с макетом,
        это и читалось как «испорчено», даже когда обрезки не было вовсе.

        При 704+40 карта не делится на 1280/1324 нацело — следующая карточка
        закономерно видна не целиком. Это не брак, а тот же приём, что и в
        самом Figma: сама карточка называется «wrapper (scroll container) for
        prototipe», то есть ряд заведомо шире экрана и подразумевает свайп —
        едва видный край следующей карточки и есть подсказка «здесь можно
        полистать», её не нужно прятать идеальным выравниванием.
      */
      className={clsx(
