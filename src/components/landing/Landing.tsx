...
/**
 * Сетка карточек: «Что входит в пакет» и «Поддержка для партнеров».
 *
 * Раньше это были горизонтальные карусели — в макете ряды нарисованы как
 * scroll container колонки. На живой странице этот приём не вытянул, и от
 * него отказались в два захода:
 *
 * 1. Ряды открывались прокрученными (смещение до 1550px из макета), и при
 *    загрузке на 1280 восемь карточек из восемнадцати не было видно вообще.
 * 2. Даже начинаясь с начала, ряд по определению режет карточку правым
 *    краем. Стрелки и градиент-подсказку пробовали — они оказались
 *    чужеродными (см. историю), а без них обрезанный край всё равно читается
 *    как брак вёрстки, а не как «здесь есть продолжение».
 *
 * Поэтому карточки просто выложены сеткой: две колонки с 1024, одна ниже.
 * Ничего не режется, ничего не спрятано, листать не нужно — а для страницы,
 * которая продаёт франшизу, видимость всех доводов важнее декоративного
 * приёма. Ширина колонки задаётся сеткой, поэтому фиксированных 704/580 у
 * карточки больше нет: на 1324 колонка выходит 642, на 1280 — 620, то есть
 * ровно тот же порядок, что в макете.
 */
function CardRails({
  items,
  variant = "package",
  ...
  items: Card[];
  /** различает только чередование заливок; геометрия у обеих секций общая */
  ...
  return (
    /*
      Появление — на СЕТКЕ, а не на карточках: один наблюдатель вместо
      восемнадцати, и ни одна карточка не может остаться прозрачной из-за
      того, что её порог пересечения не сработал (этим страдал прежний
      вариант с анимацией на каждой карточке).
    */
    <motion.div
      ...
      className={clsx(
        "grid grid-cols-1 gap-3 md:gap-5 lg:grid-cols-2 lg:gap-10",
        className,
      )}
    >
      {items.map((item, i) => (
...
function RailCard({
  ...
  /**
   * Заливки из макета: чередование стекла и плотной #262626 с кантом.
   *
   * Шахматкой, а не по чётности индекса: в сетке из двух колонок чётность
   * дала бы вертикальные полосы — левая колонка всегда одного вида, правая
   * другого. Сложение номера строки с номером карточки сдвигает рисунок в
   * каждой следующей строке, и чередование читается по обеим осям.
   */
  const checker = (Math.floor(index / 2) + index) % 2 === 1;
  const dense = support ? checker : !checker;
  const gradient = support ? checker : true;

  return (
    <motion.div
      whileHover={reduced ? undefined : { y: -6 }}
      /* h-full: ячейки сетки тянутся по самой высокой карточке в строке,
         и заливка должна занимать ячейку целиком, а не только свой текст */
      className="h-full"
    >
      <Spotlight
