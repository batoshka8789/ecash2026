  ...
  return (
    ...
    */
    <motion.div
       ...
       * ряд. По горизонтали пара -mx/px растягивает ряд до краёв ЭКРАНА:
       * (100vw − 100%)/2 — это ровно зазор от колонки до кромки вьюпорта,
       * поэтому карточки режутся о край монитора, а первая стоит на линии
       * заголовков. У обёртки лендинга overflow-x:hidden — вылет безопасен.
       *
       * scroll-pl задан ЯВНЫМИ величинами по брейкпоинтам контейнера
       * (px-4 → 52px → 40px → центрирование max-w-1324), а не тем же calc:
       * проценты в scroll-padding считаются от скроллпорта, а не от родителя,
       * (100vw−100%)/2 там равен нулю — и обязательный снап прилеплял первую
       * карточку к кромке экрана вместо линии заголовков (ловилось на 1920).
       */
      className="-mx-[calc((100vw-100%)/2)] -mt-8 -mb-24 flex snap-x snap-mandatory scroll-pl-4 flex-row gap-3 overflow-x-auto px-[calc((100vw-100%)/2)] pb-24 pt-8 [scrollbar-width:none] md:scroll-pl-[52px] md:gap-5 lg:scroll-pl-10 lg:gap-10 xl:scroll-pl-[max(0px,calc((100vw-1324px)/2))] [&::-webkit-scrollbar]:hidden"
    >
      {cards.map((item, i) => (
