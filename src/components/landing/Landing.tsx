 ...
 * Один ряд-карусель.
 *
 * Видимое окно ряда обрезано до ЦЕЛОГО числа карточек — не до 100% ширины
 * колонки. Карточка при этом остаётся ровно того размера, что в макете
 * (704/580/429), её саму не сжимаем и не растягиваем. Расхождение между
 * «сколько карточек умещается» и «сколько занимает окно» и было источником
 * обрезанного края: 704+40 не делит 1280/1324 нацело, поэтому у самого края
 * окна всегда торчал недоскроллённый огрызок следующей карточки.
 *
 * Технически: меряем реальную ширину первой отрисованной карточки и зазора
 * (а не жёстко пишем брейкпоинты повторно в JS), считаем, сколько целых
 * карточек влезает в колонку, и подгоняем под это ширину СКРОЛЛИРУЕМОГО
 * окна — с запасом (лишние миллиметры колонки) вправо пусты, а не заняты
 * половиной карточки. Свайп при этом никуда не делся: карточек в ряду
 * больше, чем видно, обрезка — просто по границе карточки, а не посередине.
 *
 * Раньше стартовое смещение бралось из макета (scrollLeft до 1550px), чтобы
 * ряды выглядели «в разбежку», как на статичном артборде, — тоже прятало
 * карточки целиком. Убрано, ряд всегда открывается с первой.
 */
function Rail({ cards, variant }: { cards: Card[]; variant: "package" | "support" }) {
  const reduced = useReducedMotion();
  /*
   * Два разных узла нарочно: frameRef всегда занимает всю колонку и меряет
   * ДОСТУПНОЕ место, windowRef — то, что мы сами сжимаем под целое число
   * карточек. Мерить сжимаемый узел собственной же шириной нельзя: после
   * первого замера он перестанет отражать реальное место в колонке и будет
   * докладывать самому себе то значение, которое сам себе и поставил.
   */
  const frameRef = useRef<HTMLDivElement | null>(null);
  const trackRef = useRef<HTMLDivElement | null>(null);
  /** null — ещё не измерено; в эту долю секунды ряд занимает всю колонку */
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (!available) return;
      /*
       * Записываем доступную ширину в CSS-переменную ДО того, как читаем
       * ширину карточки. На ширинах уже 768 карточка задана в процентах
       * (calc(100% - 18px) от track), а не в px — если сначала сжать окно и
       * только потом мерить карточку, она пересчитает свои проценты уже от
       * суженного родителя, JS увидит меньшее число и сузит окно ещё раз, и
       * так по кругу до почти нуля (натурально воспроизвелось: 217/199,
       * 307/289 — окно и карточка гонялись друг за другом вниз).
       * Переменная наследуется по дереву независимо от того, что делает
       * windowWidth с шириной промежуточных узлов, поэтому у calc() ниже
       * всегда одна и та же точка отсчёта — реальная ширина колонки.
       */
      frame.style.setProperty("--rail-frame", `${available}px`);
      const cardWidth = firstCard.getBoundingClientRect().width;
      const gap = parseFloat(getComputedStyle(track).columnGap || "0") || 0;
      if (!cardWidth) return;
      const step = cardWidth + gap;
      // сколько целых карточек влезает; минимум одна, даже если она шире колонки
      const count = Math.max(1, Math.floor((available + gap) / step));
      const next = Math.round(count * step - gap);
      setWindowWidth((prev) => (prev !== null && Math.abs(prev - next) < 1 ? prev : next));
    };
  ...
  return (
    ...
    */
    ...
    >
      <div
        // окно — ровно под целое число карточек; до первого замера на всю колонку
        style={windowWidth !== null ? { width: windowWidth } : undefined}
      ...
      >
        <div
           ...
           * ховере и её тень — на срезе видны резкие углы. -mt-8 pt-8 / -mb-24
           * pb-24 дают вертикальный запас (тень вниз до 44px, подъём 6px) и
           * гасят друг друга, не сдвигая сам ряд. Горизонтального запаса
           * специально нет: он бы сдвинул карточку от края окна, которое
           * теперь подогнано впритык под её собственную ширину.
           */
...
function RailCard({
  ...
  return (
    <motion.div
      whileHover={reduced ? undefined : { y: -6 }}
      /*
        Ширина — ТОЧНО как в макете: 704px у «Что входит в пакет», 580px у
        «Поддержки», сняты напрямую с узла cards new в дампе Figma. Внутренние
        padding 64 (p-16) и gap 44 (gap-11) у Spotlight ниже — оттуда же.

        До 768 карточка задана не в px, а через var(--rail-frame) — реальную
        ширину колонки, которую Rail пишет в CSS-переменную при каждом
        замере (см. измерение выше). Просто calc(100% - 18px) здесь нельзя:
        "100%" считался бы от track, а его ширину сам же Rail и подгоняет под
        целое число карточек — карточка пересчиталась бы от уже суженного
        родителя, JS увидел бы меньшее число и сузил ещё раз, и так по кругу
        (натурально воспроизвелось на мобильной ширине: окно и карточка
        гонялись друг за другом вниз, 217→199→…). Переменная наследуется по
        дереву в обход этой петли — резолвится всегда от истинной ширины
        колонки, а не от того, что Rail с ней сейчас делает.
      */
      className={clsx(
        "w-[calc(var(--rail-frame,100%)-18px)] shrink-0 snap-start md:w-[429px]",
