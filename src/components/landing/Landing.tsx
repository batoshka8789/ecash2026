 ...
 * Один ряд-карусель.
 *
 * Ряд идёт НАВЫЛЕТ до краёв экрана: карточки режутся о физическую кромку
 * вьюпорта, а не о границу колонки, стоящую с отступом внутри страницы.
 * Раньше следующая карточка «рождалась» из невидимой линии в нескольких
 * десятках пикселей от края монитора — это читалось как рамка, которой на
 * странице нет. Вынос — чистый CSS: отрицательные поля ровно в ширину зазора
 * «колонка → край экрана» с обеих сторон ((100vw − 100%) / 2) и такой же
 * padding, возвращающий первую карточку на линию заголовков; scroll-pl той
 * же величины, чтобы snap-start прилипал к этой же линии.
 *
 * Ширины карточек — из дампа Figma без пересчёта: 704 (пакет) / 580
 * (поддержка) с 1024, 429 на 768, во всю колонку до 768. Прошлая версия
 * делила колонку поровну (по 620), и карточка пакета переставала совпадать
 * с макетом. Теперь размеры точные, а «сколько влезает» решает сама ширина
  ...
  return (
    ...
    */
    <motion.div
       ...
       * ряд. По горизонтали -mx в (100vw − 100%)/2 растягивает скролл до
       * краёв ЭКРАНА — карточки режутся о кромку монитора и оттуда же
       * выезжают свайпом. У обёртки лендинга overflow-x:hidden, вылет
       * безопасен.
       *
       * Отступ слева — краевой отступ страницы (16 → 52 → 40), а НЕ зазор до
       * центрированной колонки. Разница проявляется на широких экранах:
       * колонка ограничена 1324 и центрируется, так что на 1920 зазор до неё
       * 298px — карточки начинались от этой линии, слева зияла пустота почти
       * в треть экрана, справа ряд упирался в кромку. Перекос, из-за которого
       * и просили сдвинуть влево; теперь слева тот же небольшой отступ, что у
       * остальной страницы, и ряд читается как полноширинная лента.
       *
       * Ноль вместо 40 (вровень с кромкой) пробовал — карточка упирается в
       * край экрана скруглённым углом и смотрится зажатой, воздух нужен.
       *
       * scroll-pl повторяет padding теми же величинами — снап обязан сажать
       * карточку на ту же линию, где стоит первая. Задан явно, а не общим
       * calc: проценты в scroll-padding считаются от скроллпорта, а не от
       * родителя, и (100vw−100%)/2 там вырождается в ноль (ловилось на 1920 —
       * снап уводил первую карточку в самую кромку экрана).
       */
      className="-mx-[calc((100vw-100%)/2)] -mt-8 -mb-24 flex snap-x snap-mandatory scroll-pl-4 flex-row gap-3 overflow-x-auto px-4 pb-24 pt-8 [scrollbar-width:none] md:scroll-pl-[52px] md:gap-5 md:px-[52px] lg:scroll-pl-10 lg:gap-10 lg:px-10 [&::-webkit-scrollbar]:hidden"
    >
      {cards.map((item, i) => (
...
function RailCard({
  ...
  return (
    <motion.div
      whileHover={reduced ? undefined : { y: -6 }}
      /*
        Ширины — точно из дампа Figma (узел cards new): 704 у «Что входит в
        пакет», 580 у «Поддержки», 429 на планшетной ширине, во всю колонку
        до 768. Никаких «долей колонки»: деление поровну меняло пропорции
        карточки (620 вместо 704), и перенос строк внутри переставал
        совпадать с макетом. Сколько карточек видно — решает ширина экрана:
        ряд идёт навылет до его кромки (см. Rail), остаток свайпается.
      */
      className={clsx(
