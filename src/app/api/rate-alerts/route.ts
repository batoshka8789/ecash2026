...
export const GET = withUser(async () => {
  ...
  try {
    ...
    /**
     * Подписки живут только в нашей базе — Ecash в этом пути не участвует,
     * поэтому единственный возможный сбой здесь её недоступность. Отдавать
     * голую 500 «Что-то пошло не так» нельзя: рядом, в блоке «Мои подписки»,
     * это читается как «с вашими подписками что-то случилось», хотя они
     * целы. 503 — «сервис временно недоступен, повторите позже».
     *
     * Пустым списком отвечать тоже нельзя, хотя так делает /api/notifications:
     * там подписки лишь добавка к заявкам, а здесь они и есть содержимое
     * экрана, и пустота выглядела бы как «подписок нет».
     */
    console.warn('[rate-alerts] список недоступен', (e as Error).message);
