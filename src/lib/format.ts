...
export function formatLoginInput(raw: string): string {
  if (/[^\d\s()+-]/.test(raw)) {
    // Почта, начинающаяся с цифр («77mail@…»), успевает получить телефонную
    // маску на первых символах — снимаем её следы, иначе в поле остаётся
    // «+7 (77mail@…». Скобок и пробелов в адресе быть не может, так что
    // чистка безопасна.
    return raw.replace(/^\+7\s*\((\d*)\)?\s*/, '$1').replace(/[()\s]/g, '');
  }
