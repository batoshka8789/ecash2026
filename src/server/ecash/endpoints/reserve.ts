...
const toUpstreamBody = (body: ReserveBody) => {
  ...
  // сумма в валюте и сумма в тенге — обе выводятся из того, что ввёл
  // клиент (value + rate); amount клиента не берём, чтобы у ядра числа
  // сходились с курсом при любом состоянии интерфейса
  const foreign = kztGive ? round2(body.value / body.rate) : body.value;
  const tenge = kztGive ? body.value : round2(body.value * body.rate);
  return {
    ...body,
