...
describe('normalizeCoords — своп перепутанных lat/lon', () => {
  ...
  it('неоднозначная пара (оба порядка похожи на Казахстан, запад страны) — тоже переворачивает', () => {
    // 47.1/51.9 — оба варианта проходят bbox (широта 40–56 пересекается
    // с долготой 46–88 в полосе 46–56), но upstream ломает координаты
    // системно, поэтому и здесь доверяем свопу, а не сырому порядку
    expect(normalizeCoords('47.1', '51.9')).toEqual({ lat: 51.9, lon: 47.1 });
  });
});
