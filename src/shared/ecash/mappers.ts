...
export function mapRequest(raw: RawRequest): ExchangeRequest {
  ...
  /**
   * Настоящий ответ казначея ПО БРОНИ. Ядро (вопреки собственной
   * документации, раздел 4: статус 8 — ПОСЛЕ подтверждения) ставит
   * «Забронирована» сразу при создании: живой пример — №6781, статус 8 при
   * acceptStatus 0, answeredAt null и пустом окне брони. Если верить одному
   * статусу, все заявки «зелёные» с первой секунды, а момент решения
   * казначея невидим. Подтверждение отличаем по фактам.
   *
   * У индивидуальной заявки факт строже: только подтверждённый accept
   * резервирования. Одного reservedUntil мало — на этапе «казначей
   * предложил курс» ядро кладёт туда окно РЕШЕНИЯ КЛИЕНТА (раздел 5,
   * шаг 3), и по нему предложение курса выглядело бы действующей бронью.
   * У обычной брони reservedUntil появляется только с подтверждением
   * (№6774) — оставляем его сигналом на случай ответа без accepts.
   */
  const bookingAcceptConfirmed = accepts.some((a) => a.actionType === 1 && a.status === 2);
  const treasurerConfirmed = isIndividual
    ? bookingAcceptConfirmed
    : iso(raw.reservedUntil) !== null || bookingAcceptConfirmed;
  ...
  // ждём решения клиента, окно 60 минут. После согласия появляется accept
  // брони (bookingRequested) — решение принято, повторно спрашивать нечего:
  // раньше это условие оставалось истинным и после согласия, кнопки
  // «Согласен/Отказываюсь» не исчезали, а повторное нажатие получало
  // 409 BOOKING_ALREADY_REQUESTED.
  // Статус — любой нетерминальный, а не только документированный 0: ядро
  // ставит 8 сразу при создании (см. treasurerConfirmed), и требование
  // «строго 0» спрятало бы предложение казначея навсегда — клиент не
  // увидел бы кнопок решения. Терминальные 1/3 исключены: там решать
  // уже нечего.
